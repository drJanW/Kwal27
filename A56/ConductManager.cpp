 ...
 */
//@@ need to explain the function of "intent"  consider rename to "request"
//@@ consider to avoid timer.restart as overly cautious or plain stupid
...
//@@some of these defines belong here, most should be moved to macros.h
...
namespace {
//@@ preceed all these cb_ 's by a comment : what are they supposed to do (and why? how?)
void cb_clockUpdate() {
...
void ConductManager::begin() {
    ...
    auto& tm = TimerManager::instance();//@@why tm here, timer elsewhere????
    ...
    bootMaster.begin();//@@strange naming: first start timers, then boot begins??!!??
//@@ insert comments with list of core modules, and why
...
void ConductManager::intentPlaySpecificFragment(uint8_t dir, int8_t file) {
  ...
  //@@ insert comment- why is this here, not in audio??.cpp  
    AudioFragment fragment{};

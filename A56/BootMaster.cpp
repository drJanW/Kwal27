 ...
 * @version 251231E
 * @date 2025-12-31
 *//@@ 2026-01-12. -- unclear what is fallback, lacks comments
 *//@@ strange use of &clock and PRTClock::instance both
 *
...
void BootMaster::cb_bootstrap() {
    ...
    if (PRTClock::instance().isTimeFetched()) {//@@ why not clock??
        cancelFallbackTimer();
        fallback.resetFlags();
//@@ was?? in wasRunning/wasFallback - ambiguous, weird
...
void BootMaster::cb_fallbackThunk() {//@@do not use thunk in a name
//@@ is this a single use bloat wrapper??
...
void BootMaster::fallbackTimeout() {
    ...
    if (!fallback.seedAttempted) {
        ...
        if (ConductManager::intentSeedClockFromRtc()) {
            ...
            // Ultimate fallback: (from Globals)
    ...
    bool wasFallback = ConductManager::isClockInFallback();//@@ do not use was - is it a singleuse bloat-wrapper???

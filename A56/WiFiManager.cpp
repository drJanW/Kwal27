 ...
 * @version 251231E
 * @date 2025-12-31
 *//@@ 2026-01-17. need for refactor - bloated
 *//@@ uses timers not using full api
 *//@@ too many theatrical single use wrappers
 *//@@ unclear overlap wifiStatus and connevct as stayus
...
static bool connected = false;//why not use muxed status64???
...
//@@ next 4 parameters need to be properly renamef and moved to globals
...
//@@ why is next line in namespace here, and not one single global entity
    TimerManager& timers()
    {
    ...
    void beginConnect()
    {
        ...
        if (!timers().isActive(cb_retryConnect)) {//@@ is this guard needed?
...
//@@ wtf is health defined as? 
//@@ is it ok to launch beginconnect from within a (closing) callback???????
    void cb_healthCheck()
    {
        if (WiFi.status() == WL_CONNECTED && WiFi.localIP() != INADDR_NONE)
            return;
...
//@@so bootWifiConnect is synonymous to beginConnect and should both be call beginWiFiConnect???
void bootWiFiConnect()
{
    beginConnect();
}

bool isWiFiConnected()//@@ should use global muxed status64!!?
{
    return connected;
}

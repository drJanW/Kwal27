...
namespace {
  ...
  void cb_luxSensorRead() {
    ...
    SensorController::setAmbientLux(lux);
...
void SensorController::addEvent(const SensorEvent& ev){
  uint8_t n = inc(qHead);
...
void SensorController::cb_sensorRead(){
  SensorController::update();
}

void SensorController::setDistanceMillimeters(float value) {
  distanceSampleMm.store(value, std::memory_order_relaxed);
  distanceSampleValid.store(true, std::memory_order_relaxed);
}

float SensorController::distanceMillimeters() {
...
void SensorController::beginDistanceSensor() {
  ...
  PL("[SensorController] DistanceSensor (VL53L1X) disabled");
#endif
}

void SensorController::beginLuxSensor() {
  // Init delay and growth configurable via globals.csv
  // Timer fires with growing interval: delay -> delay*growth -> delay*growth^2 -> ...
  I2CInitHelper::start({
      "Lux", SC_LUX,
...
void SensorController::performLuxMeasurement() {
...
void SensorController::beginSensor3() {
  ...
  PL("[SensorController] Sensor3 (board) placeholder - no hardware");
}

void SensorController::init(uint32_t ivMs)
{
  ...
  auto &tm = timers;
  if (!tm.create(baseIntervalMs, 0, SensorController::cb_sensorRead)) {
    PL("[SensorController] Failed to create sensor polling timer");
    return;
  }
  ivUpdateMs = baseIntervalMs;

  if (ivUpdateMs == 0) {
    PF("[SensorController] Polling interval not scheduled\n");
  }
}
...
void SensorController::update()
{
  ...
  if (!isnan(distanceMm)) {
    ...
    SensorController::setDistanceMillimeters(distanceMm);
    vlLastMs = now_ms();
    ...
    dist.ts_ms = vlLastMs;
    addEvent(dist);
    SENSOR_VL53_LOG("[SensorController] VL53L1X distance=%.0fmm\n", distanceMm);
  }
#endif
}

bool SensorController::readEvent(SensorEvent& out)
{

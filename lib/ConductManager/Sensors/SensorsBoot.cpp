 ...
 * reports OK/FAIL directly via NotifyConduct - no polling required.
 */
...
// No polling needed - callbacks report OK/FAIL via NotifyConduct::report()

namespace {
} // namespace

void SensorsBoot::plan() {
    // I2C already initialized in ConductManager::begin()
    // Each begin* arms its own retry timer that reports OK/FAIL via NotifyConduct

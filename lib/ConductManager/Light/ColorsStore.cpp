...
#include "PatternStore.h"  // For previewColors only
...
void ColorsStore::begin() {
    ...
    if (!colors_.empty()) {
        ...
        PF("[ColorsStore] Boot random color: %s\n", activeColorId_.c_str());
    } else {
...
String ColorsStore::buildColorsJson(const char* source) const {
...
bool ColorsStore::getColorById(const String& id, String& label, CRGB& colorA, CRGB& colorB) const {
...
String ColorsStore::getLabelForId(const String& id) const {
    if (id.isEmpty()) {
        return String();
    }
    const ColorEntry* entry = findColor(id);
...
void ColorsStore::getActiveColors(CRGB& colorA, CRGB& colorB) const {
...
bool ColorsStore::selectColor(const String& id, String& errorMessage) {
    ...
    if (id.isEmpty()) {
        ...
        PF("[ColorsStore] Color cleared to context\n");
        return true;
    }
...
bool ColorsStore::selectNextColor(String& errorMessage) {
    if (!ready_) {
        errorMessage = F("store not ready");
        return false;
    }
    ...
    PF("[ColorsStore] Color next -> %s\n", activeColorId_.c_str());
    return true;
}

bool ColorsStore::selectPrevColor(String& errorMessage) {
    if (!ready_) {
        errorMessage = F("store not ready");
        return false;
    }
...
bool ColorsStore::deleteColorSet(JsonVariantConst body, String& affectedId, String& errorMessage) {
    JsonObjectConst obj = body.as<JsonObjectConst>();
    if (obj.isNull()) {
        errorMessage = F("invalid payload");
        return false;
...
bool ColorsStore::previewColors(JsonVariantConst body, String& errorMessage)
{
    ...
    if (!parseColorPayload(colorVariant, colorA, colorB, errorMessage))
    {
        PF("[ColorsStore] previewColors reject: color read failed: %s\n",
...
bool ColorsStore::loadColorsFromSD() {
    ...
    while (csv::readLine(file, line)) {
        ...
        if (trimmed.charAt(0) == '#') {
            ...
            // Source tracking is in Conduct layer, boot always starts with source=CONTEXT
            continue;
        }
        if (!headerConsumed) {
            headerConsumed = true;
...
void ColorsStore::setDefaultLabel(const String& id, String& label) {
    label.trim();
    if (!label.isEmpty()) {
        return;
    }

 ...
 * and coordinates with ColorsStore, PatternStore, and ShiftStore.
 */
...
ShiftStore& shiftStore = ShiftStore::instance();
...
PatternStore &getPatternStore() {
    PatternStore &store = PatternStore::instance();
    if (!store.isReady()) {
        store.begin();
    }
    return store;
...
bool LightConduct::patternRead(String &payload, String &activePatternId) {
    PatternStore &store = getPatternStore();
...
bool LightConduct::colorRead(String &payload, String &activeColorId) {
    ColorsStore &store = getColorsStore();
    // Pass source from Conduct layer to store for JSON building
...
bool LightConduct::selectNextPattern(String &errorMessage) {
    ...
    NotifyRGB::stopFlashing();  // User action overrides error flash
    applyToLights();
    return true;
}
...
bool LightConduct::previewPattern(JsonVariantConst body, String &errorMessage) {
    PatternStore& patternStore = getPatternStore();
    ColorsStore& colorStore = getColorsStore();
    ...
    if (!colorVariant.isNull()) {
        CRGB colorA, colorB;
        String colorError;
        if (ColorsStore::parseColorPayload(colorVariant, colorA, colorB, colorError)) {
            params.RGB1 = colorA;
            params.RGB2 = colorB;
        } else {
...
namespace {
// HSV color shift helper (copied from ColorsStore for independence)
...
void LightConduct::applyToLights() {
    // Get RAW pattern params from PatternStore (no shifts applied)
    PatternStore& patternStore = getPatternStore();
    ...
    // Get colors from ColorsStore (RAW, no shifts)
    ColorsStore& colorsStore = getColorsStore();
    CRGB colorA, colorB;

...
void PatternStore::begin() {
    ...
    patterns_.clear();
    if (!loadFromSD()) {
        PL("[PatternStore] No CSV found - patterns empty");
    }
    ...
    if (!patterns_.empty()) {
        ...
        PF("[PatternStore] Boot random pattern: %s\n", activePatternId_.c_str());
    } else {
...
String PatternStore::buildJson(const char* source) const {
...
bool PatternStore::selectNext(String& errorMessage) {
    if (!ready_) {
        errorMessage = F("store not ready");
        return false;
    }
    ...
    PF("[PatternStore] Pattern next -> %s\n", activePatternId_.c_str());
    return true;
}

bool PatternStore::selectPrev(String& errorMessage) {
    if (!ready_) {
        errorMessage = F("store not ready");
        return false;
    }
    ...
    PF("[PatternStore] Pattern prev -> %s\n", activePatternId_.c_str());
    return true;
}
...
LightShowParams PatternStore::getActiveParams() const {
    ...
    // Return RAW params - shifts are applied in LightConduct::applyToLights()
    return entry ? entry->params : LightShowParams();
}

// setShift(), getShift(), applyShifts() REMOVED
// Pattern shifts are now applied in LightConduct::applyToLights()
...
String PatternStore::getLabelForId(const String& id) const {
    if (id.isEmpty()) {
        return String();
    }
    const PatternEntry* entry = findEntry(id);

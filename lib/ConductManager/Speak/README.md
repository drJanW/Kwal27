...
## Purpose
The Speak module is the Conduct layer for status and error audio feedback.
...
## Architecture
```
SpeakConduct::speak(intent) → PlaySentence::start(words[])
SpeakConduct::sayTime(h,m) → PlaySentence::start([HET_IS, h, UUR, m])
```
...
SpeakConduct::speak(SpeakIntent::WIFI_FAIL);      // "wifi mislukt"
SpeakConduct::speak(SpeakIntent::RTC_FAIL);       // "tijd fout"

// Speak failure via StatusComponent:
SpeakConduct::speakFail(SC_WIFI);                 // "wifi mislukt"

// Say the current time:
SpeakConduct::sayTime(14, 30);                    // "het is veertien uur dertig"

// Welcome greeting (based on hour):
SpeakConduct::speak(SpeakIntent::WELCOME);        // "goedemiddag"
```

...
AudioShiftStore& AudioShiftStore::instance() {
    static AudioShiftStore inst;
    return inst;
}

void AudioShiftStore::begin() {
    if (ready_) {
        return;
    }

    if (!SDManager::isReady()) {
        PF("[AudioShiftStore] SD not ready\n");
    ...
    while (csv::readLine(file, line)) {
        ...
        if (!headerLoaded) {
            // Read header row
            if (columns[0] != "status") {
                PF("[AudioShiftStore] CSV header must start with 'status'\n");
                break;
            }
...
void AudioShiftStore::computeMultipliers(uint64_t statusBits, float outMults[]) const {
...
std::vector<uint8_t> AudioShiftStore::getThemeBoxAdditions(uint64_t statusBits) const {
...
float AudioShiftStore::getEffectiveVolume(uint64_t statusBits) const {
...
uint16_t AudioShiftStore::getEffectiveFadeMs(uint64_t statusBits) const {
    float mults[AUDIO_PARAM_COUNT];
    computeMultipliers(statusBits, mults);

...
void LightRun::cb_measureLux() {
    ...
    static uint8_t prevShiftedHi = 0;
    static int8_t prevCalShift = 0;
    if (shiftedHi != prevShiftedHi || calendarShift != prevCalShift) {
        PF("[LightRun] Lux=%.1f calShift=%d webMultiplier=%.2f → shiftedHi=%u\n", lux, calendarShift, getWebMultiplier(), shiftedHi);
        prevShiftedHi = shiftedHi;
        prevCalShift = calendarShift;
    }
    ...
    if (Globals::luxCalSampleRequested) {
        ...
        if (LuxCalibration::instance().isFull()) {
            PL("[LuxCal] Buffer full — sample rejected");
        } else {
            // Sample = what FastLED gets, normalized by cnf*pnf
            ...
            float nf = (cnf * pnf > 0.001f) ? cnf * pnf : 1.0f;
            LuxCalSample sample;
            sample.lux         = lux;
            sample.brightness  = Globals::lastFastledBrightness / nf;
            sample.nf          = nf;
            LuxCalibration::instance().addSample(sample);
            uint8_t n = LuxCalibration::instance().sampleCount();

            bool didAutoFit = false;
            // Auto-fit when data file reaches maxLuxDataPoints — preview only, user must accept
            if (n >= Globals::maxLuxDataPoints) {
                ...
                if (LuxCalibration::instance().fitParams(fit)) {
                    ...
                    WebGuiStatus::pushLuxcalFit(
                        Globals::brMax, Globals::luxRate,
                        fit.brMax, fit.luxRate,
            ...
            // Push SSE with current count (seededLuxDataPoints if auto-fit regenerated)
            uint8_t pushCount = LuxCalibration::instance().sampleCount();
            uint8_t pushReal  = LuxCalibration::instance().realCount();
            WebGuiStatus::pushLuxcal(pushCount, pushReal, lux, Globals::lastFastledBrightness);

            // Save data file to SD (only if not just regenerated by auto-fit)
            if (!didAutoFit && !AlertState::isSdBusy()) {

...
void LightRun::cb_measureLux() {
    ...
    if (Globals::luxCalSampleRequested) {
        ...
        // Auto-fit when data file reaches maxLuxDataPoints
        if (n >= Globals::maxLuxDataPoints) {
            ...
            if (LuxCalibration::instance().fitParams(fit)) {
                ...
                if (!AlertState::isSdBusy()) {
                    LuxCalibration::instance().saveFittedParams(fit);
                    LuxCalibration::instance().generateSeeds();
                }
                PF("[LuxCal] Auto-fit at n=%u, params saved, seeds regenerated\n", n);
                didAutoFit = true;
            }
        }
        ...
        WebGuiStatus::pushLuxcal(pushCount, lux, Globals::lastUnclampedBrightness);

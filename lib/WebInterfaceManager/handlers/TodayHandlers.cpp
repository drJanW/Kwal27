/**
 * @file TodayHandlers.cpp
 * @brief Today API endpoint handlers
 * @version 251231E
 * @date 2025-12-31
 *
 * Implements HTTP handlers for the /api/context/* endpoints.
 ...
 * Integrates with TodayContext and CalendarRun for context data.
 */

#include "TodayHandlers.h"
#include "../WebUtils.h"
#include "Globals.h"
...
{
    TodayContext ctx;
    if (!calendarRun.contextRead(ctx)) {
...
{
    server.on("/api/context/today", HTTP_GET, handleToday);
}
